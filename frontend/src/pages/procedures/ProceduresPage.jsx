...
export default function ProceduresPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.code.trim() || !formData.name.trim()) {
      setFormError('Kode dan Nama Tindakan Medis wajib diisi.');
      return;
    }
    ...
    try {
      let response;
      if (isEditing) {
        response = await api.put(`/procedures/${selectedId}`, formData);
      } else {
        response = await api.post('/procedures', formData);
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alert(error.message || 'Gagal menghapus tindakan medis.');
    }
  };
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-xs mt-1">
            Kelola katalog jenis tindakan & pelayanan medis yang digunakan pada pemeriksaan Dokter (SOAP).
          </p>
        </div>
        ...
        <div className="p-4 border-b border-slate-200 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
            <span className="text-xs text-slate-500 font-medium">
              Total: <strong className="text-slate-800">{procedures.length}</strong> Tindakan Medis
            </span>
            <button
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-slate-100/70 border-b border-slate-200 text-slate-700 font-semibold uppercase tracking-wider">
                <th className="p-3.5 pl-6">Kode Tindakan</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan="3" className="text-center py-10 text-slate-400">
                    ...
                    <span>Memuat data tindakan medis...</span>
                  </td>
                </tr>
              ) : procedures.length === 0 ? (
                <tr>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 max-w-md w-full shadow-2xl relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              ...
              <div className="flex gap-3 pt-3 border-t border-slate-100">
                ...
                >
                  {submitting ? 'Saving...' : 'Simpan Tindakan'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-[100] animate-fade-in">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 max-w-sm w-full shadow-2xl text-center">
            ...
            <p className="text-slate-500 text-xs mb-6">
              Apakah Anda yakin ingin menghapus tindakan <span className="font-bold text-slate-800">{itemToDelete?.name}</span> ({itemToDelete?.code})?
            </p>
