...
export default function QueueSkipModal({ show, queue, onConfirm, onClose }) {
  if (!show || !queue) return null;
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100] animate-fade-in">
      <div className="bg-white border border-slate-200 rounded-2xl p-6 max-w-sm w-full shadow-2xl text-center">
        ...
        <h3 className="text-base font-bold text-slate-900 mb-1">Lewati Antrean Pasien?</h3>
        <p className="text-slate-500 text-xs mb-2">Apakah Anda yakin ingin melewatkan antrean:</p>
        <p className="font-bold text-slate-800 text-sm mb-1">{queue.patient_name}</p>
        <p className="font-mono text-purple-900 text-xs font-bold mb-6">(No. Antrean: {queue.queue_number})</p>
