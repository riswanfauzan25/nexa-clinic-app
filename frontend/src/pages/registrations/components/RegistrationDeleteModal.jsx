...
export default function RegistrationDeleteModal({ show, item, onConfirm, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100] animate-fade-in">
      <div className="bg-white border border-slate-200 rounded-2xl p-6 max-w-sm w-full shadow-2xl text-center">
        ...
        <h3 className="text-base font-bold text-slate-900 mb-1">Hapus Pendaftaran Pasien?</h3>
        <p className="text-slate-500 text-xs mb-2">Apakah Anda yakin ingin menghapus kunjungan:</p>
        ...
        <div className="flex gap-3 text-xs">
          ...
          <button
            type="button"
            onClick={onConfirm}
            className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white font-medium rounded-xl cursor-pointer shadow-xs transition-colors"
          >
            Ya, Hapus
          </button>
        </div>
