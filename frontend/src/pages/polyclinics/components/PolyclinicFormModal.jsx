...
export default function PolyclinicFormModal({ show, isEditing, formData, setFormData, formError, submitting, onSubmit, onClose }) {
  if (!show) return null;
        ...
        <form onSubmit={onSubmit} className="space-y-4 text-xs">
          ...
          <div className="flex gap-3 pt-3 border-t border-slate-100">
            ...
            <button type="submit" disabled={submitting} className="flex-1 py-2.5 bg-blue-800 hover:bg-blue-900 text-white font-semibold rounded-lg cursor-pointer disabled:opacity-50 shadow-xs transition-colors">
              {submitting ? 'Menyimpan...' : isEditing ? 'Perbarui Poliklinik' : 'Simpan Poliklinik'}
            </button>
          </div>
        </form>
      </div>
